...
- **Error handling**: Return errors up the stack; log fatal errors with `slog.Error()` before exiting; wrap errors with `fmt.Errorf()` and `%w` verb
...
- **Logging**: Use the standard `log/slog` package; `slog.Info()` for user-facing messages, `slog.Debug()` for debug info, and `slog.Error()` for errors. Use structured attributes and do not log tokens.
